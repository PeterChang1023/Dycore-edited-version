...
function Spectral_Dynamics!(mesh::Spectral_Spherical_Mesh,  vert_coord::Vert_Coordinate, 
  ...
  #grid_p_half .= read_file["grid_p_half_xyzt"][:,:,:,90]
  #grid_Δp .= read_file["grid_Δp_xyzt"][:,:,:,90]
  #grid_lnp_half .= read_file["grid_lnp_half_xyzt"][:,:,:,90]
  #grid_p_full .= read_file["grid_p_full_xyzt"][:,:,:,90]
  #grid_lnp_full .= read_file["grid_lnp_full_xyzt"][:,:,:,90]
...
function Get_Topography!(grid_geopots::Array{Float64, 3})
  grid_geopots .= 0.0
  #read_file = load("0404_300_50_8_variables.dat")
  #grid_geopots .= read_file["grid_geopots_xyzt"][:,:,1,90]
  return
end 
...
"""
  ...
  spe_vor_c[:,:,:] .= read_file["spe_vor_c_xyzt"][:,:,:,89]
  spe_div_c[:,:,:] .= read_file["spe_div_c_xyzt"][:,:,:,89]
  grid_u[:,:,:]    .= read_file["grid_u_c_xyzt"][:,:,:,89]
  grid_v[:,:,:]    .= read_file["grid_v_c_xyzt"][:,:,:,89]  
  ...
  grid_p_half .= read_file["grid_p_half_xyzt"][:,:,:,89]
  grid_Δp .= read_file["grid_Δp_xyzt"][:,:,:,89]
  grid_lnp_half .= read_file["grid_lnp_half_xyzt"][:,:,:,89]
  grid_p_full .= read_file["grid_p_full_xyzt"][:,:,:,89]
  grid_lnp_full .= read_file["grid_lnp_full_xyzt"][:,:,:,89]
